...
type AdminTab = 'overview' | 'users' | 'usage' | 'plans'
...
const USERS_PER_PAGE = 5
  ...
  async function changePlan(userId: string, planId: string, planName: string) {
    ...
    if (error) {
      showToast('Failed to update plan: ' + error.message)
    } else {
      showToast(`Plan updated to ${planName} successfully`)
  ...
  const mrr = subscriptions.reduce((sum, s) => {
    if (s.status === 'active' && s.plans) {
      return sum + (s.billing_cycle === 'yearly'
        ? s.plans.price_monthly * 0.83
        : s.plans.price_monthly)
    }
    return sum
  }, 0)
  ...
  function getCountryFromEmail(email?: string): string {
    ...
    if (domain?.endsWith('.io') || domain?.endsWith('.com')) return '🌐 Global'
    return '🌐 Global'
  }
              ...
              {usageLogs.length === 0 ? (
                <div className="border border-gray-100 rounded-xl p-12 text-center">
                  ...
                  <p className="text-xs text-gray-400">
                    Usage logs will appear here once users generate blueprints with token tracking enabled.
                  </p>
                </div>
              ) : (
                <div className="border border-gray-100 rounded-xl overflow-hidden">
                  ...
                  <div key={p.id} className={`border rounded-xl overflow-hidden ${
                    ...
                    <div className={`px-5 py-4 border-b ${
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        {planCounts.find(pc => pc.id === p.id)?.count || 0} active users
                      </div>
                    </div>
                    <div className="p-5 flex flex-col gap-2">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium text-black">
                          {p.blueprint_limit === -1 ? 'Unlimited' : `${p.blueprint_limit}/month`}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500">Est. COGS</span>
                        <span className="font-medium text-black">
                          {p.blueprint_limit === -1 ? 'Variable' : `$${(p.blueprint_limit * 0.11).toFixed(2)}/mo`}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium text-green-600">
                          {p.price_monthly === 0 ? '—' :
                            `${(((p.price_monthly - (p.blueprint_limit * 0.11)) / p.price_monthly) * 100).toFixed(0)}%`
                          }
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium text-black">
                          ${p.price_monthly === 0 ? '0' : (p.price_monthly * 10).toFixed(0)}/yr
                        </span>
                      </div>
                    </div>
                  </div>
