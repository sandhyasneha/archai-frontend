...
export const metadata = {
  title: 'Documentation | ArchAI',
  description: 'Complete guide to ArchAI — signing in, Greenfield infrastructure design, Brownfield migration, Knowledge Base, and plans.',
}

const nav = [
  { id: 'overview', label: 'Overview' },
  { id: 'account', label: 'Creating an account & signing in' },
  { id: 'dashboard', label: 'Dashboard' },
  { id: 'greenfield', label: 'Greenfield — new infrastructure' },
  { id: 'brownfield', label: 'Brownfield — migrate existing infrastructure' },
...
function Section({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} className="scroll-mt-20 mb-14">
      ...
      <div className="flex flex-col gap-4 text-sm leading-relaxed text-gray-700">{children}</div>
    </section>
  )
}
...
export default function DocPage() {
  return (
    <div className="min-h-screen bg-white">
      <div className="border-b border-gray-100 px-7 py-4 flex items-center justify-between sticky top-0 bg-white z-10">
        ...
        <div className="flex items-center gap-4">
          <Link href="/signin" className="text-xs text-gray-400 hover:text-black transition-colors">Sign in</Link>
          <Link href="/register" className="text-xs bg-black text-white px-3 py-1.5 rounded-md font-medium hover:opacity-85 transition-opacity">Start free</Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 flex gap-12">
        ...
        <aside className="w-56 flex-shrink-0 hidden lg:block">
          <div className="sticky top-24 flex flex-col gap-1">
            {nav.map((n) => (
              <a key={n.id} href={`#${n.id}`} className="text-xs text-gray-500 hover:text-black py-1.5 leading-snug transition-colors">
                {n.label}
              </a>
            ))}
          </div>
        ...
        <div className="flex-1 min-w-0 max-w-2xl">
          <h1 className="text-3xl font-medium tracking-tight text-black mb-2">Documentation</h1>
          <p className="text-sm text-gray-400 mb-12">Everything you need to design, audit, and migrate cloud infrastructure with ArchAI.</p>

          <Section id="overview" title="Overview">
            <p>
              ArchAI is an AI-powered cloud architecture platform with two core workflows:
            </p>
            ...
            <p>
              Both workflows run through a pipeline of specialised AI agents rather than a single prompt, so every blueprint or migration plan is reviewed and validated before you see it.
            </p>
          </Section>

          <Section id="account" title="Creating an account & signing in">
            <p>
              ArchAI is built for organisations, not individuals. You must register with a business or organisation
              email address — personal domains (Gmail, Yahoo, etc.) are not accepted.
            </p>
          ...
          <Section id="greenfield" title="Greenfield — build new infrastructure">
            <p>
          ...
          <Section id="brownfield" title="Brownfield — migrate existing infrastructure">
            <p>
              Brownfield is available on the <strong>Team plan and up</strong>. Instead of designing from scratch, you
              feed ArchAI your existing infrastructure and it produces an audit and a modernisation plan.
            </p>
            ...
            <p>
              Past migrations are listed on your Dashboard under &quot;Recent brownfield migrations&quot; — click any row to revisit it.
            </p>
          </Section>
          ...
          <Section id="knowledge-base" title="Knowledge Base">
            ...
            <p>Upload documents from the Knowledge Base page in the sidebar. Document limits scale with your plan (see below).</p>
          </Section>

          <Section id="plans" title="Plans & features">
          ...
          <Section id="settings" title="Settings & account">
            <p>
              From Settings you can update your profile, manage your Knowledge Base documents, and view your current
              plan. Organisation admins additionally see an Admin panel with user management, usage/COGS monitoring,
              and plan changes for their organisation.
            </p>
          </Section>
