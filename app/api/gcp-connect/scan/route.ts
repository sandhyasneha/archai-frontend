...
import { scanGCPWithToken, buildScanResult } from '@/lib/gcp-connect/scanner'
...
export async function POST(req: NextRequest) {
  ...
  if (!connection) {
    return Response.json({ error: 'No active GCP connection found. Connect a project first.' }, { status: 404 })
  }

  try {
    const token = await getAccessToken(connection.project_id)
    const { resources, warnings } = await scanGCPWithToken(connection.project_id, token)
    const scanResult = buildScanResult(resources)

    if (warnings.length > 0) {
      console.error('GCP connect scan warnings:', warnings)
    }

    if (scanResult.total_resources === 0) {
      return Response.json({
        error: 'no_resources_found',
        message: `No resources were found in project ${connection.project_id}. This usually means either the project is genuinely empty, or the Viewer role grant doesn't fully cover it yet.${warnings.length > 0 ? ' Details: ' + warnings.join('; ') : ''}`,
        hint: 'Confirm real resources exist in this project and that the Viewer role has been granted to ArchAI\u2019s service account, then try again.',
      }, { status: 422 })
    }
    ...
    const { data: scan, error: insertError } = await serviceClient
      ...
      .insert({
        ...
        input_content: `Live GCP connection scan: ${scanResult.total_resources} resources from project ${connection.project_id}`,
        source_cloud: 'gcp',
        target_cloud,
    ...
    if (insertError) {
      console.error('brownfield_scans insert failed (gcp-connect):', insertError)
    ...
    console.error('GCP connect scan failed:', message)
